...
public class Biz {
    ...
    public void publishContent(String uid, String content) throws IOException {
        try {
            ...
            Table contentTable = DriverUtils.getConnection().getTable(TableName.valueOf(Constants.TABLE_CONTENT));
            //组装 Rowkey
            long timestamp = System.currentTimeMillis();
            String rowKey = uid + "_" + timestamp;
            //添加微博内容
            Put put = new Put(Bytes.toBytes(rowKey));
            put.addColumn(Bytes.toBytes("info"), Bytes.toBytes(Constants.TABLE_CONTENT), timestamp,
                    Bytes.toBytes(content));
            contentTable.put(put);
            //b、向微博收件箱表中加入发布的 Rowkey
            //b.1、查询用户关系表，得到当前用户有哪些粉丝
            Table relationTable = DriverUtils.getConnection().getTable(TableName.valueOf(Constants.TABLE_RELATION));
            //b.2、取出目标数据
            ...
            //开始操作收件箱表
            Table inboxTable = DriverUtils.getConnection().getTable(TableName.valueOf(Constants.TABLE_INBOX));
        ...
        {
    /*        if (null != DriverUtils.getConnection()) {
                try {
                    DriverUtils.getConnection().close();
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }*/
    ...
    public void removeAttends(String uid, String... attends) {
        ...
        try {
            ...
            for (String attend : attends) {
                ...
                for (String att : attends) {
                    inboxDelete.addColumn(Bytes.toBytes("info"), Bytes.toBytes(att));
                }
